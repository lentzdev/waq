...
#else
boolean md5_update (register MD5 *this, register byte *buf, register word len)
#endif
{
	if (this->done)
	    return (false);
	...
	if (len > 0) {
	   ...
	   while (len >= 64) {			/* Can we do more 64 chunks? */
		 ...
		 len -= 64;			/* Substract from len left   */
	   }
...
#endif
{
	if (!this->done) {
	   ...
	   for (ptr = 0, i = 0; i < 4; i++)
		   ...
		   sprintf(((char *) this->databuf) + ptr, "%02x",
			   (short) (this->resbuf[i] >> j) & 0xff);
